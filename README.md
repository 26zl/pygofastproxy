...
- **Health endpoint** at `/health` and graceful shutdown on SIGINT/SIGTERM
...
```
Copy `.env.example` to `.env` and adjust the values to match your setup.
...
```bash
cp .env.example .env
# Set PY_BACKEND_TARGET=http://backend:4000 inside .env for Docker
docker compose up --build
```

The compose file builds the proxy and runs a simple Python `http.server` backend on port 4000.
